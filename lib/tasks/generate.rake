...
namespace :jira do
  ...
  task :generate_public_cert do
    ...
    subject = '/C=US/ST=New York/L=New York/O=SUMO Heavy Industries/CN=www.sumoheavy.com'
    system("openssl req -x509 -subj \"#{subject}\" -nodes -newkey rsa:1024 -sha1 " \
           '-keyout rsakey.pem -out rsacert.pem')
