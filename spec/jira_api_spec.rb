...
describe JiraApi::Client do

  subject {JiraApi::Client.new('foo','bar')}
  
  it "creates an instance" do
    subject.class.should == JiraApi::Client
  end
  ...
  it "sets the default options" do
    JiraApi::Client::DEFAULT_OPTIONS.each do |key, value|
  ...
  it "allows the overriding of some options" do
    ...
    client = JiraApi::Client.new('foo', 'bar', :site => 'http://foo.com/')
    client.options[:site].should == 'http://foo.com/'

    # ... but leaves the rest intact
    JiraApi::Client::DEFAULT_OPTIONS.keys.reject do |key|
      key == :site
    end.each do |key|
      client.options[key].should == JiraApi::Client::DEFAULT_OPTIONS[key]
    end

    JiraApi::Client::DEFAULT_OPTIONS[:site].should_not == 'http://foo.com/'
  end
  ...
  describe "access token" do
    ...
    it "raises an exception when accessing without initialisation" do
      ...
      end.should raise_exception(JiraApi::Client::UninitializedAccessTokenError, "init_access_token must be called before using the client")
    end
